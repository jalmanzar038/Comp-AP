...
{
    private int[]
    getSubArray(sortArray array, int begin, int size) {
        int arr[] = new
                int[size];
        for (int i = 0; i <
                size; i++) {
            arr[i] =
                    array.getValue(begin + i);
        }
        return arr;
    }

    private void
    merge(sortArray array, int
            left, int middle, int right)
    {
        ...
        while (i < leftSize
            ...
            if (leftArray[i]
                ...
                array.updateSingle(k,
                        leftArray[i], getDelay());
                i++;
            }
            else {

                array.updateSingle(k,
                        rightArray[j], getDelay());
                j++;
            }
            k++;
        }
